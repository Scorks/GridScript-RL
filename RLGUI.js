RLGUI = function (container, map) {
    ...
    self.draw = function () {
        ...
        if (self.episode.length > 0) {
            ...
            let px = parseInt((self.episode[0][0][2] + 0.5) * self.sqSize);
            let py = parseInt((self.episode[0][0][3] + 0.5) * self.sqSize);
            self.drawCircle(x, y, self.sqSize/3, '#ff0000', '#ff0000')
            self.drawCircle(px, py, self.sqSize/3, '#0000ff', '#0000ff')
        }
        ...
        if (self.episode.length > 0)
        {
            let px = self.episode[0][0][2];
            let py = self.episode[0][0][3];
            for (x = 0; x < self.env.width; x++) {
                for (y = 0; y < self.env.height; y++) {
                    for (a = 0; a < self.env.actions.length; a++) {
                        if (self.env.get(x,y) == 'X') { continue; }
                        if (self.env.get(x,y) == 'T') { continue; }
                        self.drawPolicyLine(x, y, self.env.actions[a], self.mc.P[x][y][px][py][a]);
                    }
                }
            }
        }
    ...
    self.doEpisodes = function() {
        ...
        for (i=0; i<self.mc.terminalCounter.length; i++) {
          if (self.mc.terminalCounter[i][0] == true) {
            series.push([i, self.mc.terminalCounter[i][1]]);
          }
          else {
            series.push([i, 0]);
          }
    ...
    self.setHTML = function() {
        ...
        document.getElementById('labeleps').style       = stylePrefix + ' left:' + c1l + '; top:80;   width:' + c1w + '; height:' + ch + '; ';
        document.getElementById('selecteps').style      = stylePrefix + ' left:' + c2l + '; top:80;   width:' + c2w + '; height:' + ch + '; text-align: center;';

        document.getElementById('labelepisodes').style  = stylePrefix + ' left:' + c1l + '; top:120;   width:' + c1w + '; height:' + ch + ';';
        document.getElementById('selectepisodes').style = stylePrefix + ' left:' + c2l + '; top:120;   width:' + c2w + '; height:' + ch + '; text-align: center;';
        document.getElementById('labeltime').style      = stylePrefix + ' left:' + c1l + '; top:160;   width:' + c1w + '; height:' + ch + ';';
        document.getElementById('selecttime').style     = stylePrefix + ' left:' + c2l + '; top:160;   width:' + c2w + '; height:' + ch + '; text-align: center;';
